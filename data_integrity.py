...
# Define our paths for input CSV and clean output Parquet
raw_csv_path = 'data/yellow_tripdata.csv'
parquet_path = 'data/yellow_tripdata_clean.parquet'

# Simple print to check raw file size
file_size_mb = os.path.getsize(raw_csv_path) / (1024 * 1024)
print(f"Raw CSV size: {file_size_mb:.2f} MB")

# Variables to keep track of total rows and clean rows
initial_row_count = 0
cleaned_row_count = 0

# Dictionary to hold the counts of bad data we remove
anomaly_counts = {
    "wrong_year_month": 0,
...
# Create empty lists to store sample bad rows for our report
...
for chunk_idx, df in enumerate(pd.read_csv(raw_csv_path, chunksize=chunksize)):
    ...
    # 1. Fill nulls in congestion surcharge with 0.0
    df['congestion_surcharge'] = df['congestion_surcharge'].fillna(0.0)
    
    # 2. Convert pickup and dropoff times to datetimes
    ...
    # Drop the trip_duration_min column if it exists so we keep raw column schema
